...
addpath './geom3d';     % http://www.mathworks.com/matlabcentral/fileexchange/24484-geom3d
addpath './meshes3d';   % subdir from geom3d package
addpath './xml_io';     % http://www.mathworks.com/matlabcentral/fileexchange/12907-xml-io-tools
addpath './ply_io';
addpath './yaml';
addpath './x3d';
addpath './x3d/functions';
addpath './trimbot_msgs/msggen/';
%javaaddpath('./trimbot_msgs/jar/trimbot_msgs-1.0.0.jar');
...
handles = ReadLabels('yaml',handles);
...
function PublishMarkers(handles)
%%
marray = rosmessage(handles.pubMarkers);
...
for id = 1:length(handles.objects)
  if ~isempty(handles.objects{id})
    ...
    marker.Ns = 'garden_map';
    marker.Id = id;
    marker.Type = marker.SPHERE;
    marker.Action = marker.ADD;
    marker.Lifetime = rosduration(0);
    %
    %marker.Pose.Position = rosmessage('geometry_msgs/Point');
    marker.Pose.Position.X = obj.pos(1);
    marker.Pose.Position.Y = obj.pos(2);
    marker.Pose.Position.Z = obj.pos(3) + obj.diam(3)/2;
    %
    qwxyz = eul2quat(obj.orient);
    %marker.Pose.Orientation = rosmessage('geometry_msgs/Quaternion');
    marker.Pose.Orientation.X = qwxyz(2);
    marker.Pose.Orientation.Y = qwxyz(3);
    marker.Pose.Orientation.Z = qwxyz(4);
    marker.Pose.Orientation.W = qwxyz(1);
    %
    marker.Scale.X = obj.diam(1);
    marker.Scale.Y = obj.diam(2);
    marker.Scale.Z = obj.diam(3);
    %
    %marker.Color = rosmessage('std_msgs/ColorRGBA');
    marker.Color.A = 0.8; % Don't forget to set the alpha!
    marker.Color.R = handles.labelColors(obj.label,1);
    marker.Color.G = handles.labelColors(obj.label,2);
    marker.Color.B = handles.labelColors(obj.label,3);
    
    marray.Markers = [marray.Markers; marker];
    
  end
end

handles.pubMarkers.send(marray);
disp(marray);

function PublishObjectPoints(handles)
%%
xpcl = nan(length(handles.objects),13); % xyz qxyzw dxyz rgb
...
for id = 1:length(handles.objects)
  if ~isempty(handles.objects{id})
    ...
    xpcl(id,1:3) = obj.pos;
    xpcl(id,3) = obj.pos(3)+obj.diam(3)/2;
    %
    qwxyz = eul2quat(obj.orient);
    %marker.Pose.Orientation = rosmessage('geometry_msgs/Quaternion');
    xpcl(id,4:7) = qwxyz([2 3 4 1]);
    
    xpcl(id,8:10) = obj.diam;
    xpcl(id,11:13) = handles.labelColors(obj.label,:);
  end
end
%% point cloud
xpcl = xpcl(~isnan(xpcl(:,1)),:);

mpcl = pcl2msg(xpcl(:,1:3),xpcl(:,11:13));
%mpcl = rosmessage(handles.pubMarkers);
mpcl.Header.FrameId = 'map';
mpcl.Header.Stamp = rostime('now','system');
%%
handles.pubMarkers.send(mpcl);
disp(mpcl);
...
% --------------------------------------------------------------------
function tlbRosInit_ClickedCallback(hObject, eventdata, handles)
% hObject    handle to tlbRosInit (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles = RosInit(handles);
%%
guidata(hObject,handles);

function handles = RosInit(handles)
%% draw robot
if ~isfield(handles,'robot')
  props.id = 0;
  props.label = 37;  % Robot-Vehicle
  props.pos = [0 0 0];
  props.diam = [0.7 0.5 0.3];
  props.orient = [0 0 0];
  handles.robot = DrawObject(handles, handles.shapes.Cuboid, props);
  handles.robot.FaceColor = [1 0 0.5];
  handles.robot.Tag = 'Robot';
  handles.robot.UserData.msg = [];
end
%% ros node
params = inputdlg({'ROS node name:','ROS master URI:','Robot pose refresh [s]','Robot pose topic','Map topic','Map service','Marker array topic'},...
  'Start ROS node',[1 45], ...
  {'/garden_ui','http://192.168.33.10:11311','0.1','/odometry/filtered','/garden_ui/static_map','static_map_ui','garden_ui/markers'});
if isempty(params)                  %             '/odometry/filtered'  /slam_node/base_link/odom
  return
end
...
handles.rosNode = robotics.ros.Node(params{1},params{2});
disp(handles.rosNode);
guitopic = params{1};
%% publisher
%handles.pubSimpleGoal = robotics.ros.Publisher(handles.rosNode,'/move_base_simple/goal','geometry_msgs/PoseStamped');
handles.pubSimpleGoal = robotics.ros.Publisher(handles.rosNode,[guitopic '/simple_goal'],'geometry_msgs/PoseStamped');
disp(handles.pubSimpleGoal);
handles.pubGoalObject = robotics.ros.Publisher(handles.rosNode,[guitopic '/goal_object'],'trimbot_msgs/GardenObject');
disp(handles.pubGoalObject);
handles.pubGoalObjects = robotics.ros.Publisher(handles.rosNode,[guitopic '/goal_objects'],'trimbot_msgs/GardenObjects');
disp(handles.pubGoalObjects);
handles.pubCancelGoal = robotics.ros.Publisher(handles.rosNode,[guitopic '/cancel_goal'],'std_msgs/Bool');
disp(handles.pubCancelGoal);
handles.pubOccGrid = robotics.ros.Publisher(handles.rosNode,params{5},'nav_msgs/OccupancyGrid');
disp(handles.pubOccGrid);

%handles.pubMarkers = robotics.ros.Publisher(handles.rosNode,params{7},'visualization_msgs/MarkerArray');
handles.pubMarkers = robotics.ros.Publisher(handles.rosNode,params{7},'sensor_msgs/PointCloud2');
disp(handles.pubMarkers);

%% subscriber
%handles.subCurrentPose = robotics.ros.Subscriber(handles.rosNode,params{4},'geometry_msgs/PoseStamped','BufferSize',10); %,{@ShowRobotPose, handles.robot});
handles.subCurrentPose = robotics.ros.Subscriber(handles.rosNode,params{4},'nav_msgs/Odometry','BufferSize',10); %,{@ShowRobotPose, handles.robot});
disp(handles.subCurrentPose);

%% map publisher timer
handles.tmrOccGridPeriod = str2double(params{3});
if isfield(handles,'tmrOccGrid')
  delete(handles.tmrOccGrid);
end
if handles.tmrOccGridPeriod > 0
  %   handles.tmrOccGrid = timer('TimerFcn',{@PublishOccGrid,handles}, ...
  %     'Name','PublishOccGrid','BusyMode','drop','ExecutionMode','fixedSpacing', ...
  %     'Period',handles.tmrOccGridPeriod,'StartDelay',handles.tmrOccGridPeriod, ....
  %     'StartFcn','fprintf(''PublishOccGrid: timer started\n'');' , ...
  %     'StopFcn','warning(''PublishOccGrid: timer stopped'');' );
  
  handles.tmrOccGrid = timer('TimerFcn',{@UpdateRobotPose,handles}, ...
    'Name','UpdateRobotPose','BusyMode','drop','ExecutionMode','fixedSpacing', ...
    'Period',handles.tmrOccGridPeriod,'StartDelay',handles.tmrOccGridPeriod, ....
    'StartFcn','fprintf(''UpdateRobotPose: timer started\n'');' , ...
    'StopFcn','warning(''UpdateRobotPose: timer stopped'');' );
  
  start(handles.tmrOccGrid);
else
  handles.tmrOccGrid = [];
end
...
%% map service
try
  if hasfield(handles,'svcOccupancyMap') && ~isempty(handles.svcOccupancyMap)
    delete(handles.svcOccupancyMap);
  end
  handles.svcOccupancyMap = rossvcserver(params{6},'nav_msgs/GetMap',{@RequestOccGrid,handles});
  disp(handles.svcOccupancyMap);
catch err
  warning(err.message);
end

function UpdateRobotPose(hObject, eventdata, handles)
%%
try
  msg = handles.subCurrentPose.LatestMessage;
  if ~isempty(msg)
    %disp(msg.Pose.Position);
    ShowRobotPose(hObject, msg, handles.robot);
  else
    %warning('UpdateRobotPose: no pose received!');
  end
catch err
  warning(['UpdateRobotPose: ' err.message]);
end

function ShowRobotPose(hObject, msg, robot)
%%
%disp(msg.Pose.Position);
try
  msgPose = msg.Pose;
  if hasfield(msgPose,'Pose') % from Odometry msg
    msgPose = msgPose.Pose;
  end
  if ~isempty(robot)
    %% place robot
    %disp(pos);
    props = robot.UserData;
    props.id = 0;
    props.pos = [msgPose.Position.X msgPose.Position.Y msgPose.Position.Z];
    props.diam = [0.5 0.7 0.3]; % [0.7 0.5 0.3];
    props.orient = rad2deg(quat2eul([msgPose.Orientation.W msgPose.Orientation.X msgPose.Orientation.Y msgPose.Orientation.Z]));
    props.orient(1) = props.orient(1) + 90;
    [px,py,pz] = getCuboidPos([props.pos(1) props.pos(2) props.pos(3)+props.diam(3)/2 props.diam props.orient([1 2 3])]);
    robot.Vertices(:,1) = px;
    robot.Vertices(:,2) = py;
    robot.Vertices(:,3) = pz;
    robot.UserData = props;
    %robot.UserData.msg = msg;
    %pause(0.01);
  end
catch e
  warning(e.message);
...
% --------------------------------------------------------------------
function tlbTrimObject_ClickedCallback(hObject, eventdata, handles)
% hObject    handle to tlbTrimObject (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
%%
TrimObject(hObject,handles);

function msg = MapObjectToGardenMsg(obj,msg)
%% fill GardenObject msg
...
msg.Pose.Position.X = obj.pos(1);
msg.Pose.Position.Y = obj.pos(2);
msg.Pose.Position.Z = obj.pos(3);
qr = eul2quat(obj.orient);
msg.Pose.Orientation.W = qr(1);
msg.Pose.Orientation.X = qr(2);
msg.Pose.Orientation.Y = qr(3);
msg.Pose.Orientation.Z = qr(4);

msg.Scale.X = obj.diam(1);
msg.Scale.Y = obj.diam(2);
msg.Scale.Z = obj.diam(3);

msg.Label = obj.label;
msg.Shape = obj.shape;

function TrimObject(hObject,handles)
if isempty(handles.rosNode)
  errordlg('ROS node not initialized!','Trim object');
  return;
end
%%
if isempty(handles.selectedObject)
  errordlg('Select map object first!','Trim object');
else
  if length(handles.multiSelect)<2
    %% create single msg
    obj = handles.selectedObject.UserData;
    msg = rosmessage(handles.pubGoalObject); %'trimbot_msgs/GardenObject');
    msg = MapObjectToGardenMsg(obj,msg);
    %
    handles.pubGoalObject.send(msg);
    disp(msg);
  else
    %% create multi msg
    msgs = rosmessage(handles.pubGoalObjects); %'trimbot_msgs/GardenObjects');
    for id = handles.multiSelect
      msg = rosmessage('trimbot_msgs/GardenObject');
      msg = MapObjectToGardenMsg(handles.objects{id}.UserData,msg);
      msgs.Items = [msgs.Items; msg];
    end
    %
    handles.pubGoalObjects.send(msgs);
    disp(msgs);
  end
end
...
% --------------------------------------------------------------------
function tlbMultiTrim_ClickedCallback(hObject, eventdata, handles)
% hObject    handle to tlbMultiTrim (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
if isempty(handles.rosNode)
  errordlg('ROS node not initialized!','Trim objects');
  return;
end
%%
if isempty(handles.multiSelect)
  errordlg('Select multiple map objects first (shift+click to add)!','Trim multiple objects');
else
  %% create msg
  msgs = rosmessage(handles.pubGoalObjects); %'trimbot_msgs/GardenObjects');
  for id = handles.multiSelect
...
% --------------------------------------------------------------------
function tlbNavigate_ClickedCallback(hObject, eventdata, handles)
% hObject    handle to tlbNavigate (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles = NavigateToPos(hObject,handles);

guidata(hObject,handles);

function handles = NavigateToPos(hObject,handles)
%% clik position to go to
if isempty(handles.rosNode)
  warning('ROS node not initialized!');
  return;
end
%% message
pos = ginput(1);
msg = rosmessage(handles.pubSimpleGoal); %'geometry_msgs/PoseStamped');
msg.Header.FrameId = 'map';
msg.Header.Stamp = rostime('now','system');
msg.Pose.Position.X = pos(1);
msg.Pose.Position.Y = pos(2);
handles.pubSimpleGoal.send(msg);
disp(msg);
disp(msg.Pose.Position);
%% draw marker
if isfield(handles,'goal') && ~isempty(handles.goal)
  delete(handles.goal);
  handles.goal = [];
end
props.id = 0;
props.label = 37;  % Robot-Vehicle
props.pos = [pos(1) pos(2) 0];
props.diam = [0.5 0.5 0.5];
props.orient = [0 0 0];
handles.goal = DrawObject(handles, handles.shapes.Ellipsoid, props);
handles.goal.FaceColor = [1 0 0.5];
handles.goal.Tag = 'Goal';
handles.goal.UserData.msg = msg;


function handles = CancelGoal(handles)
%% send msg to cancel navigation
msg = rosmessage(handles.pubCancelGoal); %'std_msgs/Bool');
msg.Data = true;
disp(msg);
handles.pubCancelGoal.send(msg);
%%
if isfield(handles,'goal') && ~isempty(handles.goal)
  delete(handles.goal);
  handles.goal = [];
...
function PublishOccGrid(hObject, eventdata, handles)
%% update static map with semantic region and re-publish
try
  [msg,occ] = RenderOccGrid(handles);
  %figure; occ.show;
  
  disp(msg);
  msg.Header.Seq = 0;
  msg.Header.Stamp = rostime('now','system');
  handles.pubOccGrid.send(msg);
catch e
  disp(e.message);
end

function response = RequestOccGrid(server, reqmsg, defaultrespmsg, handles)
%% service - Build the response message here
fprintf('RequestOccGrid received... '); tic;
response = defaultrespmsg;
response.Map = RenderOccGrid(handles);
fprintf('response sent. '); toc;
showdetails(response.Map);


function [occGrid, occBitmap, mapResponse] = ReadStaticMap()
%% import occ map
client = rossvcclient('/static_map');
mapResponse = call(client);
delete(client);
%% filter
msgOcc = mapResponse.Map;
occBitmap = reshape(msgOcc.Data,msgOcc.Info.Width,msgOcc.Info.Height);
occBitmap = medfilt2(occBitmap,[3 3]);
%occBitmap = imclose(occBitmap,strel('disk',2,4));
%occBitmap = medfilt2(occBitmap,[3 3]);
occBitmap = imdilate(occBitmap,strel('disk',2,4));
msgOcc.Data = occBitmap(:);
occBitmap = imrotate(occBitmap,90);
occGrid = mapResponse.Map.readOccupancyGrid;

function [response, occ] = RenderOccGrid(handles)
%% render obstacles from map to occ grid
if 1 %~isfield(handles,'occGrid') || isempty(handles.occGrid)
  if 0 %~isfield(handles,'model') || isempty(handles.model.data)
    if 0
      %% import occ map
      ...
      occ = robotics.BinaryOccupancyGrid(mapH,mapW,20);
...
if isa(occ,'robotics.BinaryOccupancyGrid')
  mapResponse.Map.writeBinaryOccupancyGrid(occ);
else
  mapResponse.Map.writeOccupancyGrid(occ);
end
mapResponse.Map.Header.FrameId = 'map';
mapResponse.Map.Header.Stamp = rostime('now','system');
mapResponse.Map.Info.Origin.Orientation.W = 1;
mapResponse.Map.Info.MapLoadTime = rostime('now','system');
...
for ti = 1:size(handles.mesh.Faces,1)
  ...
  mtx = handles.mesh.Vertices(mtf(1:3),:);
  try
...
for i = 1:length(handles.objects)
  ...
  if ~isempty(mobj)
    if hasfield(mobj,'Faces')
      ...
      %% draw object
      if tric>0
        mapObj = insertShape(mapObj,'FilledPolygon',tris(1:tric)','Opacity',1,'SmoothEdges',false);
      end
    else
...
%response.Info.Origin.Position.Z = -0.4;
%% save
if ~isempty(handles.filePath)
  fmap = (flip(reshape(response.Data,response.Info.Width,response.Info.Height)',1))==0;
  mfn = [handles.filePath(1:end-4) '.png'];
  imwrite(fmap,mfn);
  %%
  [mdir,mf,mext] = fileparts(mfn);
  yml.image = [mf mext];
  yml.resolution = 1/occ.Resolution;
  yml.origin = [occ.XWorldLimits(1) occ.YWorldLimits(1) 0]; % x y rot
  yml.negate = uint8(0);  % must be plain 0
  yml.occupied_thresh = 0.2;
  yml.free_thresh = 0.9;
  
  YAML.write(fullfile(mdir,[mf '.yaml']),yml);  
end
...
% --------------------------------------------------------------------
function tlbCancelAction_ClickedCallback(hObject, eventdata, handles)
% hObject    handle to tlbCancelAction (see GCBO)
% eventdata  reserved - to be defined in a future version of MATLAB
% handles    structure with handles and user data (see GUIDATA)
handles = CancelGoal(handles);
...
if ~isfield(handles,'occGrid') || isempty(handles.occGrid)
  if 0 %~hasfield(handles,'model') || isempty(handles.model.data)
    %% request map
    [occGrid, occ.BinMap] = ReadStaticMap();
    occ.ProbMap = occ.BinMap;
  else
...
if 1
  ...
  figure(111); clf; poccg.show(); xlim(X); ylim(Y); hold on; colormap pink;
  title('Click map object centers in selected order; right button to end');
  wobjpos = zeros(0,3);
  while true
...
switch eventdata.Key
  ...
  case 'a'
    %% publish map as markers to RViz
    PublishObjectPoints(handles);
  case 'p'
  ...
  case 'r'
    ...
    if ~isempty(handles.robot)
      UpdateRobotPose(hObject, eventdata, handles);
      %handles.tmrOccGrid
    end
    
  case '5'
    handles.selectedObject.UserData.shape = handles.shapes.Cuboid;
    guidata(hObject,handles);
  case '6'
    handles.selectedObject.UserData.shape = handles.shapes.Ellipsoid;
    guidata(hObject,handles);
  case '7'
    handles.selectedObject.UserData.shape = handles.shapes.Cylinder;
    guidata(hObject,handles);
  case '8'
    handles.selectedObject.UserData.shape = handles.shapes.Cone;
    guidata(hObject,handles);
